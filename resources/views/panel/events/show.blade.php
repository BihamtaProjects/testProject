    ...
    <div class="row">
        <div class="col-md-6"  style="padding: 20px">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('covents.update',['isEvent' => 1, $event->id] ) }}" method="post"  enctype="multipart/form-data">
                        {{csrf_field()}}
                        <div class="form-group">
                            <label> نام </label>
                            <input class="form-control" name="title" placeholder="نام" value="{{$event->title}}">
                        </div>
                        <div class="form-group">
                            <label>عکس </label>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" name="video_length" placeholder="مدت ویدیو" value="{{$event->video_length}}">
                        </div>
                        <div class="form-group">
                            <label>خلاصه</label>
                            <textarea class="ckeditor form-control" name="summary" placeholder="خلاصه" >{!!$event->summary!!}</textarea>
                        </div>
                        <div class="form-group">
                            <label>توضیحات</label>
                            <input class="ckeditor form-control" name="description" placeholder="توضیحات" value="{!!$event->description!!}">
                        </div>
                        <div class="form-group">
                            <label>پیش نیازها</label>
                            <input class="form-control" name="prerequirement" placeholder="پیش نیازها" value="{{$event->prerequirement}}">
                        </div>
                        <div class="form-group">
                            <label>مدت</label>
                            <input class="form-control" name="duration" placeholder="مدت" value="{{$event->duration}}">
                        </div>
                        <div class="form-group">
                            <label>مقیاس زمان</label>
                            <select class="form-select" aria-label="سازمان دهنده" name="unit_id">
                                <?php $mainunit = App\Models\Unit::where('id',$event->unit_id)->first() ?>
                        ...
                        <div class="form-group">
                            <?php $mainorganizer = \App\Models\Organizer::where('id',$event->organizer_id)->first();?>
                        ...
                        <div class="form-group">
                            ...
                            <input class="date-picker"  name="start_time" value="{{$event->start_time}}">
                        </div>
                        <div class="form-group">
                            <label>اولویت</label>
                            <input class="form-control" name="priority" placeholder="اولویت" value="{{$event->priority}}">
                        </div>
                        <div class="form-group">
                            <?php $maintype = \App\Models\Type::where('id',$event->type_id)->first()?>
                            <label>نوع</label>
                        ...
                        <div class="form-check">
                            ...
                            <input class="form-check-input" name="active" type="checkbox" value="1" id="status" @if($event->active==1) checked @endif>
                        ...
                        @foreach($event->faqs as $faq)
                            ...
                            <tr class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{route('coventFaq.destroy',[$event->id,$faq->id])}}">حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                        ...
                        @foreach($event->instructors as $instructor)
                          <?php $user = App\Models\User::where('id',$instructor->user_id)->first()?>
                            <tr class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{route('coventInstructor.destroy',[$event->id,$instructor->id])}}">حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    ...
                    <table class="table table-hover subject-table">
                        ...
                        @foreach($event->subjects as $subject)
                            ...
                            <tr class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{route('coventSubject.destroy',[$event->id,$subject->id])}}">حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <table class="table table-hover selected-table">
                        ...
                        <tbody>
                        <?php $row =1 ?>
                        @foreach($event->selectedgroups as $selected)
                            ...
                            <tr class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{route('coventSelectedgroup.destroy',[$event->id,$selected->id])}}">حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    ...
                    <table class="table table-hover keyword-table">
                        ...
                        @foreach($event->keywords as $keyword)
                            ...
                            <tr class="text-center">
                                ...
                                <td class="text-center">
                                    <a href="{{route('coventKeyword.destroy',[$event->id,$keyword->id])}}">حذف</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
@section('third_party_scripts')
    <script>
        ...
        $('#faq-select').change(function () {
            id = $(this).val();
            $.ajax({
                url: "{{route("ajax.covent.faq" , ['id' => $event->id])}}",
                data: {faq_id: id},
            });
            location.reload();
        });
        ...
        $('#keyword-select').change(function () {
            id = $(this).val();
            $.ajax({
                url: "{{route("ajax.covent.keyword" , ['id' => $event->id])}}",
                data: {keyword_id: id},
            });
            location.reload();
        });
        ...
        $('#subject-select').change(function () {
            id = $(this).val();
            $.ajax({
                url: "{{route("ajax.covent.subject" , ['id' => $event->id])}}",
                data: {subject_id: id},
            });
            location.reload();
        });
        ...
        $('#selected-select').change(function () {
            id = $(this).val();
            $.ajax({
                url: "{{route("ajax.covent.selected" , ['id' => $event->id])}}",
        ...
        $('#instructor-select').change(function () {
            id = $(this).val();
            $.ajax({
                url: "{{route("ajax.covent.instructor" , ['id' => $event->id])}}",
                data: {user_id: id},
            });
            location.reload();
        });
